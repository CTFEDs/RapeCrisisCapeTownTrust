...
class HeadwayVisualEditorAJAX {
	...
	public static function secure_method_save_options() {
					...
					foreach ( $element_data as $element_data_node => $element_data_node_data ) {
						...
						if ( $element_data_node == 'properties' ) {
							...
							foreach ( $element_data_node_data as $property_id => $property_value )
								HeadwayElementsData::set_property($element_id, $property_id, $property_value);
						...
						//Handle instances, states, etc.
							...
							foreach ( $element_data_node_data as $special_element => $special_element_properties )
								...
								foreach ( $special_element_properties as $special_element_property => $special_element_property_value )
									HeadwayElementsData::set_special_element_property($element_id, $special_element_type, $special_element, $special_element_property, $special_element_property_value);
			...
			/* Handle skin templates */
				...
				if ( is_array($skin_templates) && count($skin_templates) ) {
					...
					foreach ( $skin_templates as $skin_template_name => $skin_template_blocks )
						HeadwayLayout::add_template($skin_template_name, $skin_template_blocks);
		...
		//This hook is used by cache flushing, plugins, etc
	...
	public static function method_load_block_content() {
		...
		if ( is_array($unsaved_block_settings) && count($unsaved_block_settings) && isset($unsaved_block_settings['settings']) ) {
			...
			foreach ( $unsaved_block_settings['settings'] as $unsaved_block_setting_name => $unsaved_block_setting_value )
				$block['settings'][$unsaved_block_setting_name] = $unsaved_block_setting_value;
	...
	public static function method_get_element_inputs() {
		...
		HeadwayPropertyInputs::display($element, $special_element_type, $special_element_meta, $unsaved_values);
		...
		foreach ( $all_elements as $group => $group_elements ) {
			...
			foreach ( $group_elements as $element ) {
				...
				$elements[$element['id']] = array(
					...
					'states' => headway_get('states', $element, array()),
					'instances' => headway_get('instances', $element, array())
				);
	...
	public static function secure_method_add_template() {
		...
		echo json_encode(HeadwayLayout::add_template(headway_post('template_name')));
